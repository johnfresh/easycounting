...
class HistoryTableViewController: UITableViewController {
	...
	@IBAction func clearCountNumerArray(_ sender: UIBarButtonItem) {
		countNumberArray.removeAll()
		tableView.reloadData()
	}

	@IBAction func backToMainView(_ sender: UIBarButtonItem) {
		if masterView == 2{
			performSegue(withIdentifier: "historyToright", sender: self)
		}
		if masterView == 1{
	...
	override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		...
		// Configure the cell...
		cell.textLabel?.text = String(countNumberArray[indexPath.row])
